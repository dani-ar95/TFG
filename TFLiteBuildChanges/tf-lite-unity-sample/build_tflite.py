...
def build_android(enable_xnnpack = True):  # cambio
    # Main
    option_xnnpack = 'true' if enable_xnnpack else 'false'
    run_cmd(f'bazel --host_jvm_args=-Xmx30g build --jobs=6 -c opt --config=monolithic --fat_apk_cpu=arm64-v8a,armeabi-v7a,x86_64 //tensorflow/lite/java:tensorflow-lite')
    copy('bazel-bin/tensorflow/lite/java/tensorflow-lite.aar', 'Android')

    # GPU Delegate
    run_cmd('bazel --host_jvm_args=-Xmx30g build --jobs=6 -c opt --config=monolithic --fat_apk_cpu=arm64-v8a,armeabi-v7a,x86_64 //tensorflow/lite/java:tensorflow-lite-gpu')
    copy('bazel-bin/tensorflow/lite/java/tensorflow-lite-gpu.aar', 'Android')

    run_cmd('bazel --host_jvm_args=-Xmx30g build --jobs=6 -c opt --config=monolithic --fat_apk_cpu=arm64-v8a,armeabi-v7a,x86_64 //tensorflow/lite/java:tensorflow-lite-select-tf-ops')
    copy('bazel-bin/tensorflow/lite/java/tensorflow-lite-select-tf-ops.aar', 'Android')

    # Deprecated; GL Delegate
    # run_cmd('bazel --host_jvm_args=-Xmx30g build --jobs=6 -c opt --config=monolithic --copt -DMESA_EGL_NO_X11_HEADERS --copt -DEGL_NO_X11 --copt -Os --copt -DTFLITE_GPU_BINARY_RELEASE --copt -fvisibility=hidden --linkopt -s --strip always //tensorflow/lite/delegates/gpu:libtensorflowlite_gpu_gl.so')
    # copy('bazel-bin/tensorflow/lite/delegates/gpu/libtensorflowlite_gpu_gl.so', 'Android/arm64-v8a')
    # run_cmd('bazel --host_jvm_args=-Xmx30g build --jobs=6 -c opt --config=android_arm --copt -Os --copt -DTFLITE_GPU_BINARY_RELEASE --copt -fvisibility=hidden --linkopt -s --strip always //tensorflow/lite/delegates/gpu:libtensorflowlite_gpu_gl.so')
    ...
    run_cmd('bazel --host_jvm_args=-Xmx30g build --jobs=6 -c opt --config=android_arm64 --copt -Os --copt -DTFLITE_GPU_BINARY_RELEASE --copt -fvisibility=hidden --linkopt -s --strip always //tensorflow/lite/delegates/gpu/cl:libtensorflowlite_gpu_api_delegate.so')
    copy('bazel-bin/tensorflow/lite/delegates/gpu/cl/libtensorflowlite_gpu_api_delegate.so', 'Android/arm64-v8a')
    run_cmd('bazel --host_jvm_args=-Xmx30g build --jobs=6 -c opt --config=android_arm --copt -Os --copt -DTFLITE_GPU_BINARY_RELEASE --copt -fvisibility=hidden --linkopt -s --strip always //tensorflow/lite/delegates/gpu/cl:libtensorflowlite_gpu_api_delegate.so')
    copy('bazel-bin/tensorflow/lite/delegates/gpu/cl/libtensorflowlite_gpu_api_delegate.so', 'Android/armeabi-v7a')
